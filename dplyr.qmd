...
Hadley Wickham is one of the creators of all of these packages listed here: `ggplot2`, `dplyr`, and even the `tidyverse`. Because Wickham is from New Zealand, you might notice that British spellings are allowed (but not required) in many parts of the `tidyverse` landscape. Wickham lives in Houston and is currently the Chief Scientist at RStudio, which is soon changing its name to Posit.
:::
...
We can also see here that this data is a `tibble`. As far as I can tell, the name derives from a funny way to pronounce "table" in a New Zealand accent. Yes, Wickham is one of the authors of the `tibble` package, which is also part of the `tidyverse`. Just think of a `tibble` as a table or data frame with a couple added conveniences to make things nicer. Because we loaded the entire `tidyverse`, we didn't need also to load the `tibble` package individually.
:::
...
::: callout-tip
There are lots of different logical tests:
...
But this won't work at this stage because we already got rid of the `season` column. We'll come back to this problem in the next section.
...
1.  Start from the `swac` object
2.  Then filter to show only games where "Grambling" is in the `team` column and where the `season` is "2019".
3.  Then select the `date`, `team`, and `opponent` columns.
...
Notice here that the `|>` command acts like plumbing, piping things so that the first argument of each step---`filter(swac, …)`, `select(filter(swac, …), …)`---is filled in from the step that came before and doesn't need to be called explicitly. We're going to use the pipe from here on out.
...
On Mac, the keyboard shortcut is Command + M. On Windows, I think it's Control + M.
:::
...
::: callout-tip
Everybody builds up their data transformations one step at a time. It's okay to go back and edit code to add more to it.

To figure out how to get where you'd like to go, first break things down into pieces. Once you get one step working the way it should, then your next task is to figure out the next step. In this way, you earn yourself lots of little victories along the way---and a giant victory at the end!
:::
...
::: callout-note
Sometimes when you add a group with `group_by()`, the grouping persists longer than you might expect. For instance, if you group by two or more variables, a single `summarize()` step will remove the last variable grouping but keep the rest.

At the top of our results here in our last step, we can see that the data is still grouped by `season`, where it says `Groups: season [5]`. Adding an empty `ungroup()` step at the end would get things back to normal, without any groups.
:::
