...
Let's imagine we had nice summary information for the entire state in a simple, useful row, showing the percentage of statewide population each candidate earned:

```{r}
#| message: false
#| code-fold: true
louisiana_population <- 
  county_pops |> 
  filter(STNAME == "Louisiana") |> 
  select(parish = CTYNAME,
         population = POPESTIMATE2020) |> 
...
votes_statewide <- 
  ...
  mutate(parish = "Louisiana") |> 
  left_join(louisiana_population) |> 
  mutate(value = value/population) |> 
  # In the next line we *could* preserve the original column order
  # arrange(desc(value)) |> 
  pivot_wider()
```
...
Wouldn't it be nice to combine this row of data with our original parish-by-parish breakdown of all votes cast as a percentage of parish population? The `rbind()` function makes it easy to add a row in this way, as long as we have matching column names:

```{r}
votes_population_la <- rbind(votes_statewide, votes_population)

votes_population_la
```

Even though columns were ordered differently in our two data sets, they were matched perfectly by their names. The first data set determined the order of columns in the result, and its rows come on top. Here's what it looks like to reverse the order:

```{r}
votes_population_la <- rbind(votes_population, votes_statewide)

votes_population_la
```

This second version preserves the column order from `votes_population` and adds the row from `votes_statewide` at the bottom.
...
```{r}
write_csv(votes_population_la, 
          file = "data/louisiana_votes_pop.csv")
```
